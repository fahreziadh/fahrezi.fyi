    ...
    <div className="container max-w-[680px] grid sm:grid-cols-2 gap-6">
      {posts.map((post) => (
        <Link
          key={post._meta.path}
          href={`/blog/${post.slug}`}
          className="flex flex-col items-start border border-foreground/0 hover:bg-foreground/5 hover:border-foreground/10 transition-all duration-150 rounded-xl -m-3 p-3"
        >
          <Image
            src={post.thumbnail}
            alt={post.title}
