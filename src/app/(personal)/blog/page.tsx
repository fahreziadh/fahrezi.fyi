    ...
    <div className="container max-w-[680px] flex flex-col gap-6">
      {posts.map((post) => (
        <Link
          key={post._meta.path}
          href={`/blog/${post.slug}`}
          className="flex flex-row items-start border border-foreground/0 hover:bg-foreground/5 hover:border-foreground/10 rounded-xl -m-2 p-2"
        >
          <Image
            src={post.thumbnail}
            alt={post.title}
            width={60}
            height={60}
            className="aspect-square object-cover rounded-lg border border-foreground/5 shadow-sm"
          />
